...
- **言語**: TypeScript(方向決定。バージョンは環境構築時に確定)
- **パッケージ管理**: npm(Node.js v24.11.1 インストール済み)
- **主要フレームワーク/ライブラリ**: `@evenrealities/even_hub_sdk` 0.0.10(公式SDK)、`@evenrealities/evenhub-simulator` 0.7.3(公式シミュレータ)— いずれも 2026-06-12 にこのPCでの動作を検証済み。ビルドツールは Vite(公式例示構成)。UIフレームワークの要否は feat-001 の設計時に確定する
...
ソースコードのディレクトリは技術スタック選定後に追加し、本セクションを更新する。
...
技術スタック選定後に記述する(命名規則・型ヒント方針を言語に合わせて定める)。
