...
- Even G2 のHUDにタイマー情報(残り時間、現在のフェーズ、ラウンド数)を表示できる
- 運動時間・休憩時間・ラウンド数をカスタマイズしたインターバルタイマーを実行できる(タバタ式 20秒+10秒×8ラウンド を含む)
- トレーニング中にスマホ画面を見ずにタイマーの開始から終了までを完遂できる

### 背景

- Even G2 はカメラ非搭載・HUD表示特化のスマートグラスで、BLE 経由で外部アプリからテキスト/グラフィックを表示できる
- トレーニング中(特に運動フェーズ中)はスマホや時計を確認する動作が困難であり、視界内に常時タイマーが表示されていればハンズフリーで確認できる
...
- **パッケージ管理**: 未定
- **主要フレームワーク/ライブラリ**: 未定
- **詳細**: `docs/TECH_STACK.md` を参照(最初の機能案件の着手前に選定し、本セクションと `TECH_STACK.md` を更新する)
